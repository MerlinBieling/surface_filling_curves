...
def next_step(tri_mesh, iterate_point, direction, just_traversed=None, tol=1e-8):
    ...
    if type_of_iterate_point == 'vertex':
        ...
        if on_the_border == True and is_vector_in_cone(direction, vec1, vec2, vec3, tol=1e-8) == "NOT_CONTAINED":
            print("Invalid direction pointing outside of the mesh given")
            return None

        # vertice_pairs can be reduced, depending on whether just_traversed signals an edge (minus two faces)
        # or whether it signals a face (minus one face)

        if len(just_traversed) == 2:
            # we just traversed an edge, the adjacent faces won't have to be checked
            # pass
            # (Commented out to prevent syntax error)
            pass

        elif len(just_traversed) == 3:
            # we just traversed a face, the face won't have to be checked
            # pass
            # (Commented out to prevent syntax error)
            pass
        ...
        while Found_it == False and iteration_step < 30:
            # assuming there are maximum 30 faces connected to this vertex, not more crowded
            ...
            if len(vertice_pairs) == 2:
                # in this case the path segment is going from a vertex to an edge, crossing a face
                Found_it = True
        ...
        if at_border_of_mesh == True and first_step == False: 
            # this is not the first step, and we have hit the border of the mesh
            print("We are at the border, the geodesic does not continue")
            return None
        
        elif at_border_of_mesh == True and first_step == True: 
            #this is the first step and we have started at the mesh border
            face_index = iterate_point.face_index
            inward_dir = np.cross(tri_mesh.face_normals[face_index], ver1 - iterate_point.coord3d)
            dot = np.dot(direction, inward_dir)

            if dot < -tol:
                print("The point is at the border and the vector points out of the mesh. STOP")
                return None


        else:
            if just_traversed == None:
                if np.linalg.norm(np.cross(direction, edge_vec)) < tol:
                    ...
                    if dot > tol:
                        ...
                        return None
                        # probably degenerate or noise 
            else:
                # Compare sets of vertex indices to check for equality regardless of order
        ...
        vertex_index1 = iterate_point.top_indices[0]  # these are vertex indices
        vertex_index2 = iterate_point.top_indices[1]
        vertex_index3 = iterate_point.top_indices[2]
